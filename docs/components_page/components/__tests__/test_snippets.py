...
def test_snippets(dash_duo, dashr, dashjl, config):
    ...
    if path not in SKIP:
        ...
        julia_snippet = path.parent / f"{path.stem}.jl"

        if r_snippet.exists() or julia_snippet.exists():
            ...
            if r_snippet.exists():
                ...
                r_layout = requests.get(
                    f"{dashr.server_url}/_dash-layout"
                ).json()
            ...
            if julia_snippet.exists():
                # Get Julia snippet layout
                dashjl.start_server(
                    JL_WRAPPER.format(
                        snippet=julia_snippet.read_text(), name=name
                    )
                )
                julia_layout = requests.get(
                    f"{dashjl.server_url}/_dash-layout"
                ).json()

                # Test layouts match
                unittest.TestCase().assertDictEqual(
                    drop_keys(py_layout), drop_keys(julia_layout)
                )
