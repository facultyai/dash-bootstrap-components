...
def test_r_input_short_hand(dashr):
    r_app = load_r_app((HERE.parent / "input" / "short_hand.R"), "inputs")
    dashr.start_server(r_app)
    check_input_short_hand_callbacks(dashr)


def test_jl_input_short_hand(dashjl):
    jl_app = load_jl_app((HERE.parent / "input" / "short_hand.jl"), "inputs")
    dashjl.start_server(jl_app)
    check_input_short_hand_callbacks(dashjl)


def check_input_short_hand_callbacks(runner):

    wait.until(
        lambda: runner.find_element("#shorthand-output").text
        == "Checklist: [3], Selected: Option III, Radio: 3",
        timeout=10,
    )

    runner.find_element(
        "label[for='_dbcprivate_radioitems_shorthand-radio_input_2']"
    ).click()
    runner.find_element(
        "label[for='_dbcprivate_checklist_shorthand-checklist_input_2']"
    ).click()
    runner.find_element(
        "label[for='_dbcprivate_checklist_shorthand-select_input_2']"
    ).click()

    wait.until(
        lambda: runner.find_element("#shorthand-output").text
        == "Checklist: [3, 2], Selected: Option B, Radio: 2",
        timeout=10,
    )
