...
modal_scrollable_source = (HERE / "scrollable_source.py").read_text()
    ...
    return [
        ...
        html.P(
            dcc.Markdown(
                ...
                "device, they scroll independent of the page itself. If you "
                "prefer you can specify `scrollable=True` to scroll only the "
                "body of the model."
            )
        ),
        ExampleContainer(
            load_source_with_environment(
                modal_scrollable_source, "modal", {"app": app, "LOREM": LOREM}
            )
        ),
